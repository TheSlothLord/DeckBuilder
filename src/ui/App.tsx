...
function normalizeProject(data: Partial<Project>): Project {
  const deckBase = (i: number): Deck => ({
    ...
    points: [{ x: 0, y: 0 }, { x: 4000, y: 0 }, { x: 3200, y: 3000 }, { x: 800, y: 3000 }],
...
export function App() {
  ...
  const addDeck = () =>
    patch({ decks: [...project.decks, { id: `deck${Date.now()}`, label: `Deck ${project.decks.length + 1}`, shape: 'rect', length: 4000, width: 3000, notchLength: 2000, notchWidth: 1500, notchCorner: 'TR', points: [{ x: 0, y: 0 }, { x: 4000, y: 0 }, { x: 3200, y: 3000 }, { x: 800, y: 3000 }], spacing: 600, firstOffset: defaultProject.decks[0].firstOffset, noSeams: false, borderBoards: 0, cornerStyle: 'mitered', backingSpan: 'whole' }] });
      ...
      <aside className="sidebar">
        ...
        {project.decks.map((d, i) => (
          <div key={d.id} className="deck-edit">
            ...
            <Field label="Shape" hint="Deck outline. Rectangle is a plain deck; L-shape is a rectangle with a rectangular notch cut from one corner; Custom is an arbitrary polygon you enter as corner points.">
